...
dic_files = {'TRACT':["2020/data/SEMCOG_2020_control_totals_tract.csv", "TRACTID"], 
...
geo = "2020/data/Census2020_Tract_BG_BLK.csv"
...
for k, v in dic_files.items():
    ...
    df[k] = make_large_area_id(df[k], dfg)
    print(df[k].head(2))
    #df[k].drop([])
...
df["BLOCKGROUP"][["HHBASE_old", "POPBASE_old"]] = df["BLOCKGROUP"][["HHBASE", "POPBASE"]]
...
df_grp2trt = df["BLOCKGROUP"].groupby("TRACTID")[["HHBASE_old", "POPBASE_old", "HHBASE", "POPBASE"]].sum()
df["TRACT"][["HHBASE_old", "POPBASE_old", "HHBASE", "POPBASE"]] = df_grp2trt[["HHBASE_old", "POPBASE_old", "HHBASE", "POPBASE"]]
...
dict_adj_ratio={"IND": ['INDUSTRY1', 'INDUSTRY2',
       'INDUSTRY3', 'INDUSTRY4', 'INDUSTRY5', 'INDUSTRY6', 'INDUSTRY7',
       'INDUSTRY8', 'INDUSTRY9', 'INDUSTRY10', 'INDUSTRY11', 'INDUSTRY12',
       'INDUSTRY13', 'INDUSTRY14'], "EMP":['EMPWORKER']}
...
for k, v in dict_adj_total.items():
    for cen_geo in ["BLOCKGROUP", "TRACT"]:
        if v[0] in df[cen_geo].columns:
            ...
            df_new = adjust_to_new_totals(df[cen_geo][v], df[cen_geo][total]) #proportion adjust to new total "HHBASE" or "POPBASE"
            df_new = df_new.apply(lambda x: intergerize(x)) #integerize all rows and meet row totals
...
df["TRACT"].drop(["HHBASE_old", "POPBASE_old", "HHBASE", "POPBASE"], axis=1, inplace=True)
...
# %%
dict_update_cat = {
"BLOCKGROUP":["POPBASE",'AGEP1', 'AGEP2', 'AGEP3', 'AGEP4', 'AGEP5', 'AGEP6',
...
# %%
dict_update_tot={"BLOCKGROUP": ['SEX1', 'SEX2'],
"TRACT": ['INDUSTRY1', 'INDUSTRY2',
       'INDUSTRY3', 'INDUSTRY4', 'INDUSTRY5', 'INDUSTRY6', 'INDUSTRY7',
       'INDUSTRY8', 'INDUSTRY9', 'INDUSTRY10', 'INDUSTRY11', 'INDUSTRY12',
       'INDUSTRY13', 'INDUSTRY14', 'EMPWORKER']
}
